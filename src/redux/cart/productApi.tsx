...
export function getProductsFilterRestCollection(category_id: number, language: string, sorting, pageSize, currentPage = 1) {
    ...
    let data;
    if (sorting.sortBy === '') {
        data = '{products(filter:{category_id:{ eq: "' + category_id + '" } }, pageSize: ' + pageSize + ',currentPage: ' + currentPage + ' ){aggregations{attribute_code count label options{ count label value }}total_count  page_info {page_size  current_page} items { id brand name sku pending_inventory_source short_description {  html }  image { url } price_range {  minimum_price {  regular_price { value currency } final_price { value currency } fixed_product_taxes {  label amount {value  currency }}}maximum_price {  discount {  amount_off    percent_off } fixed_product_taxes { label amount { value currency } } } } }}}';
    } else {
        data = '{products(filter:{category_id:{ eq: "' + category_id + '" } }, pageSize: ' + pageSize + ',currentPage: ' + currentPage + ', sort: {' + sorting.sortBy + ': ' + sorting.sortByValue + '}){aggregations{attribute_code count label options{ count label value }}total_count  page_info {page_size  current_page} items { id brand name sku pending_inventory_source short_description {  html }  image { url } price_range {  minimum_price {  regular_price { value currency } final_price { value currency } fixed_product_taxes {  label amount {value  currency }}}maximum_price {  discount {  amount_off    percent_off } fixed_product_taxes { label amount { value currency } } } } }}}';
    }
    let query = {
        "query": data,
        "storeCode": storeId,
...
export function getProductsFilterRestCollectionProducts(category_id: number, language: string, attribute = '', value = '', sorting, pageSize, branding = '', testing = '', currentPage = 1, searchText = '') {
    ...
    if (attribute === 'price') {
        ...
        data = '{products(' + selection + ' filter:{category_id:{ eq: "' + value + '" } }, pageSize: ' + pageSize + ',currentPage: ' + currentPage + ' ' + sort + ' ){aggregations{attribute_code count label options{ count label value }}total_count  page_info {page_size  current_page} items { id brand name sku pending_inventory_source short_description {  html }  image { url } price_range {  minimum_price {  regular_price { value currency } final_price { value currency } fixed_product_taxes {  label amount {value  currency }}}maximum_price {  discount {  amount_off    percent_off } fixed_product_taxes { label amount { value currency } } } } }}}';
    }
